...
namespace POGOProtos\Data\Capture {
  ...
  final class CaptureAward extends ProtobufMessage {
    ...
    private $_unknown;
    private $activityType = array(); // repeated .POGOProtos.Enums.ActivityType activity_type = 1 [packed = true]
    private $xp = array(); // repeated int32 xp = 2 [packed = true]
    private $candy = array(); // repeated int32 candy = 3 [packed = true]
    private $stardust = array(); // repeated int32 stardust = 4 [packed = true]
    ...
    public function read($fp, &$limit = PHP_INT_MAX) {
      ...
      while(!feof($fp) && $limit > 0) {
        ...
        $field = $tag >> 3;
        switch($field) {
          case 1: // repeated .POGOProtos.Enums.ActivityType activity_type = 1 [packed = true]
            if($wire !== 2 && $wire !== 0) {
            ...
            break;
          case 2: // repeated int32 xp = 2 [packed = true]
            if($wire !== 2 && $wire !== 0) {
              throw new \Exception("Incorrect wire format for field $field, expected: 2 or 0 got: $wire");
            }
            if ($wire === 0) {
            ...
            break;
          case 3: // repeated int32 candy = 3 [packed = true]
            if($wire !== 2 && $wire !== 0) {
              throw new \Exception("Incorrect wire format for field $field, expected: 2 or 0 got: $wire");
            }
            if ($wire === 0) {
            ...
            break;
          case 4: // repeated int32 stardust = 4 [packed = true]
            if($wire !== 2 && $wire !== 0) {
              throw new \Exception("Incorrect wire format for field $field, expected: 2 or 0 got: $wire");
            }
            if ($wire === 0) {
